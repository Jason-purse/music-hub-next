 ...
 * @param path - File path relative to repo root (e.g., "content/installed-plugins/xxx/manifest.json")
...
export async function githubRead(
  ...
  // 插件资源用长缓存，其他配置用短缓存
  const isPluginAsset = path.startsWith('content/installed-plugins/')
  const revalidate = isPluginAsset ? 86400 : 30
